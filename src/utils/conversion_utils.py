...
def get_usd_value(num_coins, coin_rates, exchange_rates):
    ...
    """
    usd_rate, base_coin, exchange = get_usd_rate(coin_rates, exchange_rates)
    usd_value = convert_to_base_value(num_coins, usd_rate)
    return usd_value, base_coin, exchange


def get_usd_rate(coin_rates, exchange_rates):
    """
    ...
    :param coin_rates: {"ETH": 0.048, "BTC": 0.0094}
    :param exchange_rates: {"ETH": 1050, "BTC": 11070}
    ...
    for coin, rate in coin_rates:
        for base_coin, usd_rate in usd_rates:
            current_val = rate * usd_rates[coin]
            if current_val > current_max:
                current_max = current_val
                current_max_coin = coin

    return current_max, current_max_coin, current_max_ex
