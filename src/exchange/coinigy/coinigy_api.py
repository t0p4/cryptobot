...
class CoinigyREST:
    ...
    """
    def __init__(self):
        self.api = os.getenv('COINIGY_API_KEY', '')
        self.secret = os.getenv('COINIGY_API_SECRET', '')
        self.endpoint = os.getenv('COINIGY_API_ENDPOINT', 'https://api.coinigy.com/api/v1')
        print('hi')
    ...
    def request(self, method, query=None, json=False, **args):
        ...
        headers = {'X-API-KEY': self.api, 'X-API-SECRET': self.secret, 'Content-Type': 'application/json,application/json'}
        payload = {}
        payload.update(**args)
        if query is not None:
            payload.update(query)
        data = js.dumps(payload).encode('utf8')
        ...
        return pd.DataFrame(resp_obj['data'])
    ...
    def balances(self):
        return self.request('balances', json=True, show_nils=0, auth_ids="")
